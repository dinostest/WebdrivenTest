...
function click_func(app,module,scenario,func){
	...
	var url = "/automation/loadscenario?app="+app+ "&module=" + module + "&scenario=" + scenario + "&func=" + func;
		...
		if (csv.thread){
			...
			$("#"+tableId).handsontable({
				...
				afterOnCellMouseDown: function(event,coords,TD){
					console.log(coords);
					var caseitem = {};
					for (var x in csv.header){
						caseitem[csv.header[x]] = curData[coords.row][x];
					}
...
function change_content(keywords,content,tableId){
	teststeps = [];
	for (var i = 0; i < keywords.length; i++){
		items = keywords[i].split("_");
		for (var j = 1; j < items.length; j = j + 2){
			items[j] = "\"" + content[items[j]] + "\"";
		}
		teststeps.push(items.join(""));
	}
	html = "<ul>";
	for (var i = 0; i < teststeps.length; i++){
		html = html + "<li>" + teststeps[i] + "</li>";
	}
	html = html + "</ul>";
	$("#scenario_description").html(html);
	$("#" + tableId).handsontable("render");
}
